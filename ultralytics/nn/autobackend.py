...
def check_class_names(names):
    ...
    if isinstance(names, dict):
        ...
        #     names_map = yaml_load(YOLO_ROOT / "configs/datasets/yolos/ImageNet.yaml")["map"]  # human-readable names
...
class AutoBackend(nn.Module):
    ...
    def __init__(
        ...
        if "names" not in locals():  # names missing
            raise ValueError(
                f"Model has no class names. Please provide a data.yaml file with 'names' key or "
                f"specify 'names' in the AutoBackend constructor."
            )
