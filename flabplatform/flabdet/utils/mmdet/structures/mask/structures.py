...
import torch
from flabplatform.flabcv.ops.roi_align import roi_align
...
class BitmapMasks(BaseInstanceMasks):
    ...
    def rescale(self, scale, interpolation='nearest'):
        ...
        if len(self.masks) == 0:
            new_w, new_h = flabplatform.flabcv.rescale_size((self.width, self.height), scale)
            rescaled_masks = np.empty((0, new_h, new_w), dtype=np.uint8)
        else:
            rescaled_masks = np.stack([
                flabplatform.flabcv.imrescale(mask, scale, interpolation=interpolation)
    ...
    def flip(self, flip_direction='horizontal'):
        ...
        if len(self.masks) == 0:
            ...
            flipped_masks = np.stack([
                flabplatform.flabcv.imflip(mask, direction=flip_direction)
                for mask in self.masks
            ])
    ...
    def pad(self, out_shape, pad_val=0):
        ...
        if len(self.masks) == 0:
            ...
            padded_masks = np.stack([
                flabplatform.flabcv.impad(mask, shape=out_shape, pad_val=pad_val)
                for mask in self.masks
            ])
    ...
    def translate(self,
        ...
        if len(self.masks) == 0:
            ...
            if masks.shape[-2:] != out_shape:
                ...
                masks = empty_masks
            translated_masks = flabplatform.flabcv.imtranslate(
    ...
    def shear(self,
        ...
        if len(self.masks) == 0:
            ...
            sheared_masks = flabplatform.flabcv.imshear(
    ...
    def rotate(self,
        ...
        if len(self.masks) == 0:
            ...
            rotated_masks = flabplatform.flabcv.imrotate(
                self.masks.transpose((1, 2, 0)),
                angle,
                center=center,
                scale=scale,
