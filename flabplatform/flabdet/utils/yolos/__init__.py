...
HELP_MSG = """
    Examples for running Flabplatform:
    ...
    3. Use the command line interface (CLI):
        ...
        Flabplatform 'yolo' CLI commands use the following syntax:
...
class TQDM(rich.tqdm if TQDM_RICH else tqdm.tqdm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        This class extends the original tqdm class to provide customized behavior for Flabplatform projects.
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            ...
            {YOLO_DEFAULT_CFG_PATH} with the latest version from
...
DEFAULT_CFG_DICT = yaml_load(YOLO_DEFAULT_CFG_PATH)
...
def get_user_config_dir(sub_dir="Flabplatform"):
    """
...
USER_CONFIG_DIR = Path(os.getenv("YOLO_CONFIG_DIR") or get_user_config_dir())  # Flabplatform settings dir
...
class TryExcept(contextlib.ContextDecorator):
    """
    Flabplatform TryExcept class. Use as @TryExcept() decorator or 'with TryExcept():' context manager.
...
class SettingsManager(JSONDict):
    """
    SettingsManager class for managing and persisting Flabplatform settings.
    ...
    def __init__(self, file=SETTINGS_FILE, version="0.0.6"):
        ...
        with torch_distributed_zero_first(RANK):
            ...
            if not self.file.exists() or not self:  # Check if file doesn't exist or is empty
                LOGGER.info(f"Creating new Flabplatform Settings v{version} file ✅ {self.help_msg}")
                self.reset()
    ...
    def _validate_settings(self):
        ...
        if not (correct_keys and correct_types and correct_version):
            LOGGER.warning(
                "WARNING ⚠️ Flabplatform settings reset to default values. This may be due to a possible problem "
        ...
        if self.get("datasets_dir") == self.get("runs_dir"):
            LOGGER.warning(
                f"WARNING ⚠️ Flabplatform setting 'datasets_dir: {self.get('datasets_dir')}' "
    ...
    def update(self, *args, **kwargs):
        ...
        for k, v in kwargs.items():
            if k not in self.defaults:
                raise KeyError(f"No Flabplatform setting '{k}'. {self.help_msg}")
            t = type(self.defaults[k])
            if not isinstance(v, t):
                raise TypeError(
                    f"Flabplatform setting '{k}' must be '{t.__name__}' type, not '{type(v).__name__}'. {self.help_msg}"
                )
        super().update(*args, **kwargs)
...
def vscode_msg(ext="ultralytics.ultralytics-snippets") -> str:
    """Display a message to install Flabplatform-Snippets for VS Code if not already installed."""
    path = (USER_CONFIG_DIR.parents[2] if WINDOWS else USER_CONFIG_DIR.parents[1]) / ".vscode/extensions"
    obs_file = path / ".obsolete"  # file tracks uninstalled extensions, while source directory remains
    installed = any(path.glob(f"{ext}*")) and ext not in (obs_file.read_text("utf-8") if obs_file.exists() else "")
    url = "https://docs.ultralytics.com/integrations/vscode"
    return "" if installed else f"{colorstr('VS Code:')} view Flabplatform VS Code Extension ⚡ at {url}"


# Run below code on utils init ------------------------------------------------------------------------------------

# Check first-install steps
PREFIX = colorstr("Flabplatform: ")
