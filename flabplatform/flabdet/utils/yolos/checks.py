...
def parse_requirements(file_path=YOLO_ROOT.parent / "requirements.txt", package=""):
    """
...
@TryExcept()
def check_requirements(requirements=YOLO_ROOT.parent / "requirements.txt", exclude=(), install=True, cmds=""):
    """
...
def check_file(file, suffix="", download=True, download_dir=".", hard=True, target_dir=None):
    """
    ...
    if (
        ...
        if not target_dir:
            files = glob.glob(str(YOLO_ROOT / "**" / file), recursive=True) or glob.glob(str(YOLO_ROOT.parent / file))  # find file
        else:
            files = glob.glob(str(target_dir / "**" / file), recursive=True)
...
def check_yaml(file, suffix=(".yaml", ".yml"), hard=True, target_dir=None):
    """
    ...
    """
    return check_file(file, suffix, hard=hard, target_dir=target_dir)  # check suffix
...
def git_describe(path=YOLO_ROOT):  # path must be a directory
    """
...
def print_args(args: Optional[dict] = None, show_file=True, show_func=False):
    ...
    try:
        file = Path(file).resolve().relative_to(YOLO_ROOT).with_suffix("")
    except ValueError:
        file = Path(file).stem
