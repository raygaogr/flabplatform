...
from flabplatform.flabdet.utils.yolos import LOCAL_RANK, NUM_THREADS, TQDM, LOGGER
...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        self.update_labels()  # single_cls and include_class
        ...
        # Transforms
        self.transforms = self.build_transforms(augment_args=augment_args)
    ...
    def build_transforms(self, augment_args=None):
        """
...
class LabelMeDataset(BaseDataset):
    ...
    def build_transforms(self, augment_args=None):
        if self.augment: # mosaic和mixup需要关掉rect模式
            augment_args.mosaic = augment_args.mosaic if self.augment and not self.rect else 0.0
            augment_args.mixup = augment_args.mixup if self.augment and not self.rect else 0.0
            transforms = v8_transforms(self, self.imgsz, augment_args)
        else: # 验证阶段
        ...
        transforms.append(
            Format(
                ...
                batch_idx=True,
                mask_ratio=augment_args.mask_ratio,
                mask_overlap=augment_args.overlap_mask,
                bgr=augment_args.bgr if self.augment else 0.0,  # only affect training.
            )
        )
        return transforms
