...
import numpy as np
from flabplatform.flabcv.image import imresize
from flabplatform.flabcv.image.geometric import _scale_size
from flabplatform.flabcv.transforms import BaseTransform
from flabplatform.flabcv.transforms import Pad as MMCV_Pad
...
from flabplatform.flabdet.utils.mmdet.structures.bbox import HorizontalBoxes, autocast_box_type
from flabplatform.flabdet.utils.mmdet.structures.mask import BitmapMasks, PolygonMasks
from flabplatform.flabdet.utils.mmdet import log_img_scale
    ...
    def _resize_img(self, results):
        ...
        if results.get('img', None) is not None:
            if self.keep_ratio:
                ...
                img, w_scale, h_scale = flabplatform.flabcv.imresize(
                    results['img'],
                    results['scale'],
                    interpolation=self.interpolation,
                    return_scale=True,
...
class FixShapeResize(Resize):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if self.keep_ratio:
            ...
            img, scale_factor = flabplatform.flabcv.imrescale(
...
class RandomFlip(MMCV_RandomFlip):
    ...
    def _flip(self, results: dict) -> None:
        ...
        if results.get('gt_seg_map', None) is not None:
            results['gt_seg_map'] = flabplatform.flabcv.imflip(
...
class SegRescale(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if self.scale_factor != 1:
            results['gt_seg_map'] = flabplatform.flabcv.imrescale(
                results['gt_seg_map'],
                self.scale_factor,
...
class MixUp(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        retrieve_img = flabplatform.flabcv.imresize(
        ...
        out_img = flabplatform.flabcv.imresize(out_img, (int(out_img.shape[1] * jit_factor),
