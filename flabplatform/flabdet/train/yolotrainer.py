...
class DetectionTrainer(BaseTrainer):
    ...
    def build_dataset(self, img_path, mode="train", batch=None):
        ...
        dataset_cfg["single_cls"] = self.args.single_cls or False
        dataset_cfg["stride"] = int(gs)
        dataset_cfg["pad"] = 0.0 if mode == "train" else 0.5
        dataset_cfg["prefix"] = colorstr(f"{mode}: ")  # prefix for logging
        dataset_cfg["task"] = self.args.task
        dataset_cfg["classes"] = self.args.classes
        dataset_cfg["data"] = self.data
        dataset_cfg["fraction"] = self.args.fraction if mode == "train" else 1.0
    ...
    def get_model(self, cfg=None, weights=None, verbose=True):
        ...
        """
        model_cfg = {}
        model_cfg["type"] = "DetectionModel"
        model_cfg["nc"] = self.data["nc"]
        model_cfg["verbose"] = verbose and RANK == -1
        model_cfg["cfg"] = cfg
    ...
    def get_validator(self):
        ...
        validator_cfg["dataloader"] = self.test_loader
        return VALIDATORS.build(validator_cfg)
