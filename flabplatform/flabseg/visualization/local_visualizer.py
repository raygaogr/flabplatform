...
from flabplatform.flabseg.registry import VISUALIZERS
from flabplatform.flabseg.structures import SegDataSample
from flabplatform.flabseg.utils import get_classes, get_palette
...
class SegLocalVisualizer(Visualizer):
    ...
    def _draw_sem_seg(self,
        ...
        if with_labels:
            ...
            for mask_num in range(len(labels)):
                ...
                # print(classes_color, loc, text)
                # print('mask,', mask.shape, mask.dtype)
                # print('loc,', loc[0] + label_width + baseline,
                #       loc[1] + label_height + baseline)
                
                mask = cv2.rectangle(np.ascontiguousarray(mask), loc,(loc[0] + label_width + baseline,loc[1] + label_height + baseline),classes_color, -1)
    ...
    def set_dataset_meta(self,
        ...
        if dataset_name is None:
            dataset_name =  'a0belt_images'#'cityscapes'
    ...
    def add_datasample(
        ...
        if out_file is not None:
            flabplatform.flabcv.imwrite(flabplatform.flabcv.rgb2bgr(drawn_img), out_file)
        else:
