...
import cv2
import flabplatform.flabcv
import mmengine
import numpy as np
from flabplatform.flabcv.transforms import RandomFlip as CV_RandomFlip
from flabplatform.flabcv.transforms import Resize as CV_Resize
from flabplatform.flabcv.transforms.base import BaseTransform
from flabplatform.flabcv.transforms.utils import cache_randomness
from mmengine.utils import is_tuple_of
from numpy import random
from scipy.ndimage import gaussian_filter

from flabplatform.flabseg.datasets.dataset_wrappers import MultiImageMixDataset
from flabplatform.flabseg.registry import TRANSFORMS
...
class ResizeToMultiple(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        img = results['img']
        img = flabplatform.flabcv.imresize_to_multiple(
            img,
            self.size_divisor,
            scale_factor=1,
        ...
        for key in results.get('seg_fields', []):
            gt_seg = results[key]
            gt_seg = flabplatform.flabcv.imresize_to_multiple(
                gt_seg,
                self.size_divisor,
                scale_factor=1,
...
class CLAHE(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        for i in range(results['img'].shape[2]):
            results['img'][:, :, i] = flabplatform.flabcv.clahe(
...
class RandomCrop(BaseTransform):
    ...
    def __init__(self,
        ...
        super().__init__()
        # print('crop_size: ', crop_size, type(crop_size))
...
class RandomRotate(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if rotate:
            # rotate image
            results['img'] = flabplatform.flabcv.imrotate(
                results['img'],
                angle=degree,
            ...
            # rotate segs
            for key in results.get('seg_fields', []):
                results[key] = flabplatform.flabcv.imrotate(
                    results[key],
                    angle=degree,
...
class AdjustGamma(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        results['img'] = flabplatform.flabcv.lut_transform(
    ...
    def transform(self, results: dict) -> dict:
        ...
        for key in results.get('seg_fields', []):
            if self.scale_factor != 1:
                results[key] = flabplatform.flabcv.imrescale(
...
class PhotoMetricDistortion(BaseTransform):
    ...
    def saturation(self, img: np.ndarray) -> np.ndarray:
        ...
        if random.randint(2):
            img = flabplatform.flabcv.bgr2hsv(img)
            img[:, :, 1] = self.convert(
                img[:, :, 1],
                alpha=random.uniform(self.saturation_lower,
                                     self.saturation_upper))
            img = flabplatform.flabcv.hsv2bgr(img)
        return img
...
class RandomRotFlip(BaseTransform):
    ...
    def random_rotate(self, results: dict) -> dict:
        ...
        results['img'] = flabplatform.flabcv.imrotate(results['img'], angle=angle)
        for key in results.get('seg_fields', []):
            results[key] = flabplatform.flabcv.imrotate(results[key], angle=angle)
        return results
...
@TRANSFORMS.register_module()
class RandomFlip(CV_RandomFlip):
    ...
    def _flip(self, results: dict) -> None:
        ...
        # flip image
        results['img'] = flabplatform.flabcv.imflip(
    ...
    def _resize_seg(self, results: dict) -> None:
        ...
        for seg_key in results.get('seg_fields', []):
            if results.get(seg_key, None) is not None:
                if self.keep_ratio:
                    gt_seg = flabplatform.flabcv.imrescale(
                        results[seg_key],
                        results['scale'],
                        interpolation='nearest',
                        backend=self.backend)
                else:
                    gt_seg = flabplatform.flabcv.imresize(
                        results[seg_key],
                        results['scale'],
                        interpolation='nearest',
                        backend=self.backend)
...
class RandomMosaic(BaseTransform):
    ...
    def _mosaic_transform_img(self, results: dict) -> dict:
        ...
        for i, loc in enumerate(loc_strs):
            ...
            img_i = flabplatform.flabcv.imresize(
    ...
    def _mosaic_transform_seg(self, results: dict) -> dict:
        ...
        for key in results.get('seg_fields', []):
            ...
            for i, loc in enumerate(loc_strs):
                ...
                scale_ratio_i = min(self.img_scale[0] / h_i,
                                    self.img_scale[1] / w_i)
                gt_seg_i = flabplatform.flabcv.imresize(
                    gt_seg_i,
