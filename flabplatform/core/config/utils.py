...
# (module import from) and package name (used by pip install).
...
def _get_package_and_cfg_path(cfg_path: str) -> Tuple[str, str]:
    ...
    assert package in MODULE2PACKAGE, (
        f'flabplatform does not support to load {package} config.')
...
def _is_builtin_module(module_name: str) -> bool:
    ...
    if module_name.startswith('.'):
        return False
    if module_name.startswith('flabplatform.utils.config'):
        return True
...
class ImportTransformer(ast.NodeTransformer):
    ...
    :class:`flabplatform.utils.config.LazyObject` and preload the base variable before
    ...
    To avoid this problem, we introduce :class:`flabplatform.utils.config.LazyObject` and
    :class:`flabplatform.utils.config.LazyAttr`. Before we proceed with further
    ...
    Args:
        ...
        base_dict (dict): All variables defined in base files.
            ...
            Examples:
                >>> from flabplatform.utils.config import read_base
                >>>
                >>>
                >>> with read_base():
