...
class Config:
    ...
    Examples:
        ...
        >>> cfg.filename
        "/home/username/projects/flabplatform/tests/data/config/a.py"
        >>> cfg.item4
        'test'
        >>> cfg
        "Config [path: /home/username/projects/flabplatform/tests/data/config/a.py]
        :"
    ...
    def merge_from_dict(self,
        ...
        Examples:
            >>> from flabplatform import Config
    ...
    def _is_lazy_import(filename: str) -> bool:
        ...
        for node in ast.walk(parsed_codes):
            ...
            if isinstance(node, ast.ImportFrom):
                ...
                if node.level != 0:
                    return True
                # Skip checking when using `flabplatform.utils.config` in cfg file
                if (node.module == 'flabplatform' and len(node.names) == 1
