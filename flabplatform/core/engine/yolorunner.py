...
from flabplatform.flabdet.models import (
    ClassificationModel,
    OBBModel,
    PoseModel,
    SegmentationModel,
...
class YOLORunner(BaseRunner):
    ...
    def __init__(
        ...
        self.mode = self.overrides["mode"]  # mode type
        if self.mode == "train":
...
class YOLORunnerA(BaseRunner):
    """
    A class for implementing YOLO models, unifying APIs across different model types.
    ...
    def __init__(
        ...
        self.overrides = overrides  # model args
        self.task = self.overrides["task"]  # task type
        self.mode = self.overrides["mode"]

        if self.mode == "predict":
            self.predictor = self.build_predictor()
        elif self.mode == "export":
            self.exporter = self.build_exporter()
        else:
            self.trainer = self.build_trainer()  # build trainer for training
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        Examples:
            ...
            ...     print(f"Detected {len(r)} objects in image")
        """
        return super().__call__(source, stream=stream, **kwargs)
    ...
    def _load(self, weights: str, task=None) -> None:
        ...
        if Path(weights).suffix == ".pt":
            ...
            self.task = self.model.args["task"]
            self.model.args = self._reset_ckpt_args(self.model.args)
            if self.overrides["resume"] == False:
                self.model.args = self.overrides
            else:
                self.overrides = self.model.args = {**self.overrides, **self.model.args}
            # self.overrides = {**self.overrides, **self.model.args}
            self.ckpt_path = self.model.pt_path
        else:
            weights = checks.check_file(weights)  # runs in all cases, not redundant with above call
            self.model, self.ckpt = weights, None
    ...
    def build_predictor(self):
        ...
        predictor_cfg["overrides"] = args
        predictor_cfg["_callbacks"] = self.callbacks
        predictor = PREDICTORS.build(predictor_cfg)
    ...
    def predict(
        ...
        **kwargs: Any,
        ...
        args = {**self.overrides, "mode": "predict", **kwargs}  # highest priority args on the right
        prompts = args.pop("prompts", None)  # for SAM-type models
    ...
    def build_trainer(self, **kwargs: Any) -> None:
        model = str(self.overrides["model"]).strip()
        ...
        if Path(model).suffix == ".pt":
            self._load(model, task=self.task)  # load model from checkpoint
            if args.get("resume"):
                args["resume"] = self.ckpt_path  # resume training from checkpoint
            else:
                self.model = trainer.get_model(weights=self.model if self.ckpt else None, cfg=self.model.yaml)
        else:
            if not Path(model).suffix:
                model = model + ".yaml"
            model_cfg = yaml_model_load(model)  # load model from YAML file
            self.model = trainer.get_model(weights=None, cfg=model_cfg)
