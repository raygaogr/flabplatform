...
from flabplatform.core.config.utils import MODULE2PACKAGE
...
class Registry:
    ...
    def infer_scope() -> str:
        ...
        if module is not None:
            ...
            # use "flabplatform" to handle some cases which can not infer the scope
            # like initializing Registry in interactive mode
            scope = 'flabplatform'
            print_log(
                'set scope as "flabplatform" when scope can not be inferred. You '
    ...
    def switch_scope_and_registry(self, scope: Optional[str]) -> Generator:
        ...
        Examples:
            >>> from flabplatform.utils.registry import Registry, DefaultScope, MODELS
            >>> import time
            >>> # External Registry
    ...
    def build(self, cfg: dict, *args, **kwargs) -> Any:
        ...
        Examples:
            >>> from flabplatform import Registry
            >>> MODELS = Registry('models')
            >>> @MODELS.register_module()
            >>> class ResNet:
